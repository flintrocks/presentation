...
% a very simple example of one - written in Flint - that allows to deposit and withdraw cryptocurrency,
% in this case Ether or its smaller denomination, Wei. At the core is a contract
% that interacts with the world by receiving currency from and sending currency
% to other addresses, in this case, ANYone may send currency to this contract - using the deposit method -,
% but the only way currency is withdrawn is if the WITHDRAW method is called -
% which may only be done by the OWNER. In the real world smart contracts are
% far more complicated and will often interact with each other.
\begin{frame}
...
% Problem statement
%
% Before we go into the details of our work, let's take a step back................
% Ethereum is a distributed application platform on the blockchain.
% Users may execute arbitrary code snippets running on the Ethereum 
% Virtual Machine, EVM for short; we just saw an example of such a code snippet;
% these are what we call smart contracts.
...
% PHP of smart contract programming languages, as it has few safety features.
% Flint is an attempt at creating a less error prone programming
% language. The underpinning philosophy is simplicity, and its design has been largely inspired by
% Swift. Creating a safe language involves 
...
\end{itemize}
% Easy integration: integrate into the project with minimal code changes
% Low boilerplating: Automate the majority of the workflow.
% Easy-to-use API: Writing tests should be familiar and intuitive, API similar to established frameworks.
\end{frame}
...
\begin{itemize}
    \item \mintinline{swift}{a.(b.(c.d))} vs \mintinline{swift}{((a.b).c).d}
\end{itemize}
\end{frame}
...
% \frametitle{Toolchain}
% \begin{figure}[H]
%     \caption*{LSP client running in VSCode}
%     \centering
%     \includegraphics[width=0.5\textwidth]{figures/lsp-use.png}
% \end{figure}
% \end{frame}
